...
IpmSolution interiorPointMethod(std::unique_ptr<ProblemInstance> &prob, double epsilon) {
    ...
    A.block(0, 0, n-1, n) = -J;
    A.block(0, n, n-1, 2*n) = -J;
    A.block(n-1, 0, n+k-1, n) = F;
    A.block(n-1, n, n+k-1, 2*n) = -F;
    A.block(n-1, 2*n, n+k-1, 2*n+k) = -Eigen::MatrixXd::Identity(k, k);
    ...
    for (int t = 0; t < 5; t++) { // TODO
        ...
        rhs.head(x.size()) = rp;
        rhs.segment(x.size(), x.size() + y.size()) = rd;
        ...
        Eigen::VectorXd dy = d.segment(x.size(), x.size() + y.size());
